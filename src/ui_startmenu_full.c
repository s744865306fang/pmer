...
static void OpenPCFromFullStartMenu(void)
{
    EnterPokeStorage(2);
}

static void Task_OpenPCFromStartMenu(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        StartMenuFull_FreeResources();
        PlayRainStoppingSoundEffect();
        CleanupOverworldWindowsAndTilemaps();
        SetMainCallback2(OpenPCFromFullStartMenu);
    }
}
...
static void Task_StartMenuFullMain(u8 taskId)
{
    ...
    if (JOY_NEW(R_BUTTON))
    {
        PlaySE(SE_SELECT);
        BeginNormalPaletteFade(PALETTES_ALL, 0, 0, 16, RGB_BLACK);
        gTasks[taskId].func = Task_OpenPCFromStartMenu;
    }
